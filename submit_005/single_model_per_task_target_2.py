...
def generate_single_task_model(dir_data_train, dir_data_val, 
                               dir_model_01s, dir_model_06s, dir_model_12s, dir_model_03s,
                               dir_out, target, balance=False, dir_features=None):
    str_type = target
    # set output file footer
    str_footer = "fusion_single_" + str_type
    ...
    """
    data_train_01s.reset_index(inplace=True, drop=True)
    data_train_06s.reset_index(inplace=True, drop=True)
    data_train_12s.reset_index(inplace=True, drop=True)
    data_train_03s.reset_index(inplace=True, drop=True)
    data_val_01s.reset_index(inplace=True, drop=True)
    data_val_06s.reset_index(inplace=True, drop=True)
    data_val_12s.reset_index(inplace=True, drop=True)
    data_val_03s.reset_index(inplace=True, drop=True)
    """
    ...
    np_train_true_12s, pred_train_ens_12s, np_val_true_12s, pred_val_ens_12s = predict_data(data_train_12s, data_val_12s,
                                                                                            model_12s, str_type, 
                                                                                            window_time, dir_features)
    log = "01s pred shape) train: {0}, val: {1}".format(pred_train_ens_12s.shape, pred_val_ens_12s.shape)
    print(log)
